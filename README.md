...
Milestones 1–23: a working relational store, queryable in SQL — in
...
(`INSERT ... ON CONFLICT`), descending index columns, EXPLAIN, and
...
- **SQL frontend** — the `sql` package parses, plans, and executes a
  ...
  (with `ON CONFLICT` upsert, SERIAL identity columns, and constant
  column DEFAULTs),
...
- [ ] Later: order-aware path selection among redundant indexes, window frames (`ROWS/RANGE BETWEEN`), `LAG`/`LEAD`/`FIRST_VALUE`, standalone `CREATE SEQUENCE`
