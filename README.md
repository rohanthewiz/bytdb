...
ALTER TABLE t ALTER [COLUMN] c SET DEFAULT expr | DROP DEFAULT
ALTER TABLE t ALTER [COLUMN] c SET NOT NULL | DROP NOT NULL
ALTER TABLE t ALTER [COLUMN] c [SET DATA] TYPE type [USING expr]  -- rewrites the table
ALTER TABLE t ALTER [COLUMN] c SET STORAGE mode | SET STATISTICS n -- accepted, no-op
ALTER TABLE t DROP CONSTRAINT t_pkey, ADD PRIMARY KEY (a [, ...])  -- re-keys the table
ALTER TABLE t OWNER TO role         -- accepted, no-op (bytdb has no roles)
CREATE [UNIQUE] INDEX [IF NOT EXISTS] idx ON t (c [ASC|DESC], ...)
...
`columns` (views included), `sequences`, `table_constraints`, and `key_column_usage`
