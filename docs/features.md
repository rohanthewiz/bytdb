...
- `ALTER TABLE t ADD COLUMN c type` — **O(1)** without a `DEFAULT`: no rows are
  ...
  non-empty table too.
- `ALTER TABLE t ADD COLUMN c int GENERATED BY DEFAULT AS IDENTITY` (or
  `SERIAL`) numbers the existing rows 1..n in primary-key order and leaves the
  counter at n+1, so the next insert draws n+1 — the same result as Postgres,
  whose numbering follows heap order instead. O(rows), atomic, and capped by
  the same backfill limit as a `DEFAULT` backfill. *(verified in
  `alter_rebuild_test.go`, `sql/alter_test.go`)*
...
- `ALTER TABLE t ALTER [COLUMN] c [SET DATA] TYPE type [USING expr]` rewrites
  every row — and the row keys when `c` is a key column, and every index that
  orders by it — in one atomic transaction. Without `USING`, only the casts
  Postgres applies automatically are allowed: int ↔ float (float → int rounds
  half to even), date ↔ timestamp, any type → text, and `varchar(n)` length
  changes; anything else fails with Postgres's `column "c" cannot be cast
  automatically to type ...` (SQLSTATE 42804) and a `USING c::type` hint.
  `USING` may read any column of the old row and may fill NULLs; its result
  coerces like an inserted value (so a text result parses into the new type).
  CHECK constraints are re-checked against the converted rows, a conversion
  that makes two keys collide fails with `could not create unique index`
  (23505), and a column `DEFAULT` is cast along with the column — if it cannot
  be, the statement fails (drop the default first). Refused: a column on
  either side of a foreign key (drop the constraint first) and an identity
  column to a non-int type. `SET STORAGE`, `SET COMPRESSION`, and `SET
  STATISTICS` are accepted as no-ops (bytdb has no TOAST and no planner
  statistics), so pg_dump output runs unmodified.
- **Replacing the primary key** uses Postgres's two-action spelling:
  `ALTER TABLE t DROP CONSTRAINT t_pkey, ADD [CONSTRAINT n] PRIMARY KEY (a, b)`.
  A bytdb table always has a primary key, so this pairing is the only
  multi-action `ALTER TABLE` accepted, and runs as one atomic re-key of every
  row and index entry. The new key's columns become `NOT NULL` and so do the
  old key's (Postgres leaves that behind too). It fails, changing nothing, on a
  NULL in a new key column (23502), on two rows sharing a new key
  (`could not create unique index "t_pkey"`, `Key (a)=(1) is duplicated.`),
  or when another table's foreign key depends on the old key — create a unique
  index on the referenced columns first. A lone `ADD PRIMARY KEY` fails as it
  does in Postgres: `multiple primary keys for table "t" are not allowed`.
- `ALTER COLUMN TYPE`, the key replacement, and identity `ADD COLUMN` rewrite
  the whole table in one transaction, so they hold every row live until
  commit and are refused over `DefaultBackfillLimit` like a `DEFAULT`
  backfill. *(verified in `alter_rebuild_test.go`, `sql/alter_test.go`)*
