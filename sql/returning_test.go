...
func TestReturningRejects(t *testing.T) {
	...
	// A trailing RETURNING with no items is a parse error, not an
	// empty clause.
	if _, err := d.Exec(`insert into t values (1, 'a') returning`); err == nil {
		t.Fatal("empty RETURNING list: want a parse error")
	}
