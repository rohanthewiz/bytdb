...
//
//	CREATE TABLE [IF NOT EXISTS] t (id int PRIMARY KEY, name text NOT NULL,
//	       price int CHECK (price > 0), ...)
//	CREATE TABLE t (a int, b int, ..., PRIMARY KEY (a, b),
//	       [CONSTRAINT name] CHECK (expr), ...)
//	CREATE TABLE t (id serial PRIMARY KEY, ...)
//	       (serial = int GENERATED BY DEFAULT AS IDENTITY + NOT NULL)
...
//	SELECT [DISTINCT] * | items FROM tables [WHERE ...] [GROUP BY c, ...] [HAVING ...]
...
// !=, <>, <, <=, >, >=, the regex operators ~, !~, ~*, !~* (Go
// regexp syntax), and [NOT] LIKE / [NOT] ILIKE (% and _ wildcards;
// backslash, the only supported escape, makes the next pattern
// character literal), in either operand order, plus IS [NOT] NULL,
// [NOT] IN (list), [NOT] BETWEEN [SYMMETRIC] — sugar for the
// comparisons it is defined as, usable anywhere they are, CHECK
// constraints included — EXISTS (SELECT ...), and comparison ANY|ALL
// over an array value or a subquery (IN (SELECT ...) lowers to
// = ANY; an empty set is FALSE for ANY, TRUE for ALL, NULL operands
// leave the result unknown); in HAVING an operand may be an
...
//
// SELECT DISTINCT deduplicates the final result rows — after
// grouping, HAVING, and projection — by the same equivalence GROUP BY
// uses, so NULLs collapse to one row. The deduplicated result has
// only output columns, so ORDER BY must name one of them (or a
// select-list position), never a FROM column the projection dropped.
// DISTINCT ON is not supported; the aggregate-argument form
// (COUNT(DISTINCT x)) is, per the aggregates above.
//
// Window functions — fn(...) OVER (PARTITION BY ... ORDER BY ...
// [frame]) in the select list or ORDER BY — emit one value per input
// row instead of folding rows, so a query using one materializes its
// post-filter rows, partitions them (GROUP BY's equivalence again),
// sorts each partition by the window ORDER BY, and computes per row.
// ROW_NUMBER, RANK, and DENSE_RANK rank; LAG, LEAD, FIRST_VALUE,
// LAST_VALUE, and NTH_VALUE surface another row's value; and the
// regular aggregates work as window aggregates (SUM(x) OVER ...,
// DISTINCT arguments excluded). Frame-sensitive functions — the
// aggregates and FIRST/LAST/NTH_VALUE — evaluate over each row's
// frame: an explicit ROWS, RANGE (offsets are distances on the sort
// key), or GROUPS clause with optional EXCLUDE CURRENT ROW/GROUP/
// TIES, or the SQL default, RANGE UNBOUNDED PRECEDING .. CURRENT ROW,
// which with ORDER BY ends at the current row's last peer — so
// LAST_VALUE is the last peer, not the partition end, the same
// surprise Postgres serves.
//
...
// bytes; timestamp[tz] [with|without time zone], date, uuid; text[];
// jsonb, with json an alias for it). The
...
//
// text[] is the one array type: values read and write in Postgres's
// array text form ('{a,"b c"}'), elements test with the ANY/ALL
// comparisons (tag = ANY(tags)), and array_length(a, 1) and
// array_to_string(a, delim [, nullstr]) work on them; ARRAY(SELECT
// ...) renders its rows in the same text form. jsonb stores each
// document canonicalized on write and compares (=, ordering, keys)
// by that canonical text. The Postgres operator family works over
// it: -> and ->> access a field or element, #> and #>> follow a
// path, || concatenates, - deletes, and @>, <@, ?, ?|, ?& are
// containment and key-existence predicates, riding the same
// three-valued predicate machinery as the comparisons. Postgres's
// error/NULL split is kept: accessors answer NULL for any miss —
// absent key, index out of range, wrong container kind — while
...
// pg_am, pg_database, pg_roles, and pg_stat_activity (one row per
// backend, fed by the wire server via SetActivityProvider; empty on
...
//
// An int column may be an identity column — GENERATED BY DEFAULT AS
// IDENTITY, or the serial pseudo-types (serial, bigserial, ..., which
// also imply NOT NULL) — filled from a durable per-column counter
// when an INSERT omits it. An explicit value inserts as given and,
// when ahead of the counter, bumps it past itself (MySQL's collision
// rule), so later draws never collide with restored or imported rows.
// GENERATED ALWAYS is rejected rather than accepted-and-weakened:
// it forbids explicit inserts, which this engine does not enforce.
// Drawn keys read back through RETURNING or lastval()/currval()
// (below); TRUNCATE ... RESTART IDENTITY resets the counters.
//
...
// In the default single-writer engine, allocation is transactional,
// unlike Postgres: a rolled-back nextval's value is handed out again.
// Under bytdb.WithConcurrentWrites, sequence and identity draws
// switch to Postgres's non-transactional, gap-tolerant semantics —
// an aborted or retried transaction burns its values, and setval
// takes effect immediately — because otherwise every concurrent
// insert into a table would conflict on its counter key and
// optimistic concurrency would buy nothing. INSERT VALUES entries
// are full expressions — nextval('s') directly in VALUES works, as
// does drawing the key first (SELECT nextval, the pattern drivers
// use) — though column references have no input row to resolve
// against there, as in Postgres.
//
...
// BEGIN accepts the standard transaction modes: READ ONLY is
// honored, and isolation levels mean what the engine mode makes
// them. In the default single-writer engine every write transaction
// is serializable by construction, satisfying any requested level; a
// writable block holds the writer lock from BEGIN to its end, writes
// in other sessions wait behind it, and reads never do. Under
// bytdb.WithConcurrentWrites, writable blocks instead run
// concurrently with optimistic validation at snapshot isolation
// (Postgres REPEATABLE READ in spirit): first committer wins, and a
// losing COMMIT fails with bytdb.ErrTxConflict — SQLSTATE 40001 with
// Postgres's message and retry hint over the wire — for the client
// to retry from BEGIN. Blocks are never retried server-side (the
// client has seen their reads), but autocommit statements are, a few
// times silently, before a conflict surfaces — safe because no
// partial result has reached the client. BEGIN ISOLATION LEVEL
// SERIALIZABLE, SET TRANSACTION before the block's first query, or
// default_transaction_isolation = serializable opts a transaction up
// to SERIALIZABLE via read-set validation, autocommit writes
// included; SHOW transaction_isolation reports the engine's real
// mode (repeatable read under concurrent writes, serializable
// otherwise). DDL cannot run inside a block in either mode, because
// the engine gives every schema change its own transaction; under
// concurrent writes DDL escalates to a brief exclusive stall rather
// than ever losing validation, so it always makes progress and never
// returns 40001.
package sql
...
func (d *DB) dispatch(st Statement, args []any) (*Result, error) {
	...
	case *CreateTable:
		...
		return &Result{}, nil
	case *DropTable:
		// IF EXISTS resolves here rather than in the engine: a missing
		// table becomes a notice, anything else (e.g. a dependent
		// foreign key) still errors from the engine call below.
		if s.IfExists && d.e.Table(s.Table) == nil {
			return &Result{Notice: `table "` + s.Table + `" does not exist, skipping`}, nil
		}
